...
export class ChartJsPanelCtrl extends MetricsPanelCtrl {
  ...
  getColIndex(name, panel, opt_isOptional?: boolean) {
    let { colIndexesByText } = this.data;
    let key = name + "ColumnName";
    let isRequired = !opt_isOptional || ![null, undefined].includes(panel[key]);
  ...
  getChartOptions(chartType) {
    ...
    if (chartType === "funnel") {
      let sortMap = measures
        .map((v, i) => ({ v, i }))
        .sort(sortOrder === "desc" ? (a, b) => b.v - a.v : (a, b) => a.v - b.v)
        .map(({ i }) => i);
    ...
    let bgColors = baseColors.map(color =>
      Color(color)
        .a(panel.dataBgColorAlpha)
        .rgba()
    );
    let borderColors = baseColors.map(color =>
      Color(color)
        .l(panel.dataBorderBrightness)
        .a(panel.dataBorderColorAlpha)
        .rgba()
    );
  ...
  drawPieChart(canvas) {
    ...
    let datasets = series.map((seriesName, seriesIndex) => {
      ...
      return {
        ...
        datalabels: {
          ...
          backgroundColor: Color(panel.labels.isBlackText ? "white" : "black")
            .a(0.75)
            .rgba(),
    ...
    if (DISABLE_ANIMATIONS) {
      (chartConfig.options as any).animation = false;
    }

    if (panel.labels.isShowing) {
      (chartConfig as any).plugins = [ChartDataLabels];
    }
    ...
    url = url
      ...
      .replace(RGX_CELL_PLACEHOLDER, function(
        ...
        if (isTime) {
          ...
          return (
            (opt_timePart !== "to" ? "from=" + encodeURIComponent(from) : "") +
            (opt_timePart ? "" : "&") +
...
export { ChartJsPanelCtrl as PanelCtrl };
